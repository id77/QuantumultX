...
const CryptoJS = initCryptoJS();
const md5 = initMD5();

const eapiKey = 'e82ckenh8dichen8';

$.cookie = $.getData('id77_wyyCookie');

if ($.isNode()) {
  $.cookie = process.env.WYY_COOKIE || $.cookie;
}
...
async function 点歌(params) {
  let song;
  if ($.cookie) {
    song = await 搜索(params);
    if (!song) song = await 搜索1(params);
  } else {
    song = await 搜索1(params);
  }
...
async function 搜索(params) {
  return new Promise((resolve, reject) => {
    try {
      const url = `https://interface3.music.163.com/eapi/search/song/list/page`;
      const headers = getNEHeaders(extractCookies($.cookie));
      const encryptedParams = eapiEncrypt('/api/search/song/list/page', {
        keyword: params.name,
        limit: 20,
        offset: 0,
        scene: 'normal',
        needCorrect: '1',
        e_r: true,
        // header: getParamsHeader(extractCookies($.cookie)),
        header: {},
      });
      const body = `params=${encryptedParams}`;

      // console.log(eapiDecrypt(encryptedParams));
      ...
      $.post(opts, (err, resp, data) => {
        ...
        try {
          // 使用跨平台兼容的 getRawBody 获取二进制数据
          const rawBody = getRawBody(resp);
          if (!rawBody) {
            console.log(`搜索失败: 无法获取二进制响应数据`);
            resolve('');
            return;
          }
          const result = JSON.parse(eapiDecryptBytes(rawBody)?.raw)?.data
            ?.resources?.[0]?.baseInfo?.simpleSongData;
          if (result.id) {
            console.log(`搜索成功`);
            resolve({
              id: result.id,
...
function getNeKey() {
  return CryptoJS.enc.Utf8.parse(eapiKey);
}

function neEncrypt(text) {
  const key = getNeKey();
  if (!key) return '';
...
/**
 * AES-ECB 解密 (与 neEncrypt 对应)
 * @param {string} cipherHex - 大写 HEX 字符串
 * @returns {string} 解密后的明文
 */
function neDecrypt(cipherHex) {
  const key = getNeKey();
  if (!key) return '';

  // HEX 字符串转为 CryptoJS 格式
  const cipherParams = CryptoJS.lib.CipherParams.create({
    ciphertext: CryptoJS.enc.Hex.parse(cipherHex.toLowerCase()),
  });
...
/**
 * 纯 JS 实现: 二进制数据转 HEX 字符串
 * 兼容 Buffer / Uint8Array / ArrayBuffer
 * @param {Buffer|Uint8Array|ArrayBuffer} bytes - 二进制数据
 * @returns {string} 大写 HEX 字符串
 */
function bytesToHex(bytes) {
  // 如果是 Buffer 且有 toString 方法
  if (
    bytes &&
    typeof bytes.toString === 'function' &&
    bytes.constructor?.name === 'Buffer'
  ) {
    return bytes.toString('hex').toUpperCase();
  }
  // Uint8Array / ArrayBuffer / 普通数组
  let arr = bytes;
 ...
 * @param {Buffer|Uint8Array|ArrayBuffer} cipherBytes - 加密的二进制数据
 * @returns {object} 解密后的对象 { url, text, digest }
 */
function eapiDecryptBytes(cipherBytes) {
  // 使用纯 JS 的 bytesToHex 转换
  const cipherHex = bytesToHex(cipherBytes);
  return eapiDecrypt(cipherHex);
}

/**
 * 备用解密方法 (使用 Node.js crypto)
 */
function eapiDecryptBytesAlt(cipherBytes) {
  try {
    ...
    return parseEapiData(decrypted);
  } catch (e) {
    console.error('eapiDecryptBytesAlt error:', e.message);
    return null;
  }
}
...
/**
 * 解析 eapi 解密后的数据格式
 * 格式: ${url}-36cd479b6b5-${text}-36cd479b6b5-${digest}
 * @param {string} decryptedStr - 解密后的字符串
 * @returns {object} { url, text, digest, data }
 */
function parseEapiData(decryptedStr) {
  const separator = '-36cd479b6b5-';
  const parts = decryptedStr.split(separator);

  if (parts.length === 3) {
    const [url, text, digest] = parts;
    let data = text;
 ...
 */
function eapiDecrypt(cipherHex) {
  try {
    const decryptedStr = neDecrypt(cipherHex);
    return parseEapiData(decryptedStr);
  } catch (e) {
    console.error('eapiDecrypt error:', e.message);
    return null;
  }
}

/**
 * 跨平台获取二进制响应数据 (不依赖 Buffer)
 * - QX/Surge/Loon: bodyBytes (Uint8Array)
 * - Node.js: rawBody (Buffer)
 * @param {object} resp - 响应对象
 * @returns {Uint8Array|Buffer|null} 二进制数据
 */
function getRawBody(resp) {
  if (!resp) return null;
  // Node.js 使用 rawBody (Buffer)
  if (resp.rawBody) return resp.rawBody;
  // QX/Surge/Loon 使用 bodyBytes (Uint8Array)
  if (resp.bodyBytes) return resp.bodyBytes;
  // 如果都没有，尝试从 body 字符串转换
  if (resp.body && typeof resp.body === 'string') {
    // 尝试作为 HEX 字符串解析
    if (/^[0-9A-Fa-f]+$/.test(resp.body)) {
      return hexToBytes(resp.body);
    }
  }
  return null;
}
...
function getNEHeaders(cookies) {
  ...
  return {
    ...
    Cookie: cookieStr,
    'User-Agent': `neteasemusic/${cookies.appver || '9.4.55'} (iPhone; iOS 17.0; Scale/3.00)`,
  ...
  return (
    (Md5.prototype.update = function (t) {
      if (!this.finalized) {
        ...
        for (
          var s, i, h = 0, o = t.length, f = this.blocks, a = this.buffer8;
          h < o;
          ...
          if (
            ...
            if (ARRAY_BUFFER)
              for (i = this.start; h < o && i < 64; ++h) a[i++] = t[h];
            else
              for (i = this.start; h < o && i < 64; ++h)
                f[i >> 2] |= t[h] << SHIFT[3 & i++];
          else if (ARRAY_BUFFER)
            for (i = this.start; h < o && i < 64; ++h)
              (s = t.charCodeAt(h)),
    ...
    (Md5.prototype.hash = function () {
      ...
      this.first
        ? ((t = o[0] - 680876937),
          ...
          (s = (-1732584194 ^ (2004318071 & t)) + o[1] - 117830708),
          (s = (((s << 12) | (s >>> 20)) + t) << 0),
          (e = (-271733879 ^ (s & (-271733879 ^ t))) + o[2] - 1126478375),
          (e = (((e << 17) | (e >>> 15)) + s) << 0),
          (r = (t ^ (e & (s ^ t))) + o[3] - 1316259209),
          (r = (((r << 22) | (r >>> 10)) + e) << 0))
        : ((t = this.h0),
          (r = this.h1),
          (e = this.h2),
          (s = this.h3),
          (t += (s ^ (r & (e ^ s))) + o[0] - 680876936),
          (t = (((t << 7) | (t >>> 25)) + r) << 0),
          (s += (e ^ (t & (r ^ e))) + o[1] - 389564586),
          (s = (((s << 12) | (s >>> 20)) + t) << 0),
          (e += (r ^ (s & (t ^ r))) + o[2] + 606105819),
          (e = (((e << 17) | (e >>> 15)) + s) << 0),
          (r += (t ^ (e & (s ^ t))) + o[3] - 1044525330),
          (r = (((r << 22) | (r >>> 10)) + e) << 0)),
        (t += (s ^ (r & (e ^ s))) + o[4] - 176418897),
        (t = (((t << 7) | (t >>> 25)) + r) << 0),
        (s += (e ^ (t & (r ^ e))) + o[5] + 1200080426),
        (s = (((s << 12) | (s >>> 20)) + t) << 0),
        (e += (r ^ (s & (t ^ r))) + o[6] - 1473231341),
        (e = (((e << 17) | (e >>> 15)) + s) << 0),
        (r += (t ^ (e & (s ^ t))) + o[7] - 45705983),
        (r = (((r << 22) | (r >>> 10)) + e) << 0),
        (t += (s ^ (r & (e ^ s))) + o[8] + 1770035416),
        (t = (((t << 7) | (t >>> 25)) + r) << 0),
        (s += (e ^ (t & (r ^ e))) + o[9] - 1958414417),
        (s = (((s << 12) | (s >>> 20)) + t) << 0),
        (e += (r ^ (s & (t ^ r))) + o[10] - 42063),
        (e = (((e << 17) | (e >>> 15)) + s) << 0),
        (r += (t ^ (e & (s ^ t))) + o[11] - 1990404162),
        (r = (((r << 22) | (r >>> 10)) + e) << 0),
        (t += (s ^ (r & (e ^ s))) + o[12] + 1804603682),
        (t = (((t << 7) | (t >>> 25)) + r) << 0),
        (s += (e ^ (t & (r ^ e))) + o[13] - 40341101),
        (s = (((s << 12) | (s >>> 20)) + t) << 0),
        (e += (r ^ (s & (t ^ r))) + o[14] - 1502002290),
        (e = (((e << 17) | (e >>> 15)) + s) << 0),
        (r += (t ^ (e & (s ^ t))) + o[15] + 1236535329),
        (r = (((r << 22) | (r >>> 10)) + e) << 0),
        (t += (e ^ (s & (r ^ e))) + o[1] - 165796510),
        (t = (((t << 5) | (t >>> 27)) + r) << 0),
        (s += (r ^ (e & (t ^ r))) + o[6] - 1069501632),
        (s = (((s << 9) | (s >>> 23)) + t) << 0),
        (e += (t ^ (r & (s ^ t))) + o[11] + 643717713),
        (e = (((e << 14) | (e >>> 18)) + s) << 0),
        (r += (s ^ (t & (e ^ s))) + o[0] - 373897302),
        (r = (((r << 20) | (r >>> 12)) + e) << 0),
        (t += (e ^ (s & (r ^ e))) + o[5] - 701558691),
        (t = (((t << 5) | (t >>> 27)) + r) << 0),
        (s += (r ^ (e & (t ^ r))) + o[10] + 38016083),
        (s = (((s << 9) | (s >>> 23)) + t) << 0),
        (e += (t ^ (r & (s ^ t))) + o[15] - 660478335),
        (e = (((e << 14) | (e >>> 18)) + s) << 0),
        (r += (s ^ (t & (e ^ s))) + o[4] - 405537848),
        (r = (((r << 20) | (r >>> 12)) + e) << 0),
        (t += (e ^ (s & (r ^ e))) + o[9] + 568446438),
        (t = (((t << 5) | (t >>> 27)) + r) << 0),
        (s += (r ^ (e & (t ^ r))) + o[14] - 1019803690),
        (s = (((s << 9) | (s >>> 23)) + t) << 0),
        (e += (t ^ (r & (s ^ t))) + o[3] - 187363961),
        (e = (((e << 14) | (e >>> 18)) + s) << 0),
        (r += (s ^ (t & (e ^ s))) + o[8] + 1163531501),
        (r = (((r << 20) | (r >>> 12)) + e) << 0),
        (t += (e ^ (s & (r ^ e))) + o[13] - 1444681467),
        (t = (((t << 5) | (t >>> 27)) + r) << 0),
        (s += (r ^ (e & (t ^ r))) + o[2] - 51403784),
        (s = (((s << 9) | (s >>> 23)) + t) << 0),
        (e += (t ^ (r & (s ^ t))) + o[7] + 1735328473),
        (e = (((e << 14) | (e >>> 18)) + s) << 0),
        (r += (s ^ (t & (e ^ s))) + o[12] - 1926607734),
        (r = (((r << 20) | (r >>> 12)) + e) << 0),
        (i = r ^ e),
        (t += (i ^ s) + o[5] - 378558),
        (t = (((t << 4) | (t >>> 28)) + r) << 0),
        (s += (i ^ t) + o[8] - 2022574463),
        (s = (((s << 11) | (s >>> 21)) + t) << 0),
        (h = s ^ t),
        (e += (h ^ r) + o[11] + 1839030562),
        (e = (((e << 16) | (e >>> 16)) + s) << 0),
        (r += (h ^ e) + o[14] - 35309556),
        (r = (((r << 23) | (r >>> 9)) + e) << 0),
        (i = r ^ e),
        (t += (i ^ s) + o[1] - 1530992060),
        (t = (((t << 4) | (t >>> 28)) + r) << 0),
        (s += (i ^ t) + o[4] + 1272893353),
        (s = (((s << 11) | (s >>> 21)) + t) << 0),
        (h = s ^ t),
        (e += (h ^ r) + o[7] - 155497632),
        (e = (((e << 16) | (e >>> 16)) + s) << 0),
        (r += (h ^ e) + o[10] - 1094730640),
        (r = (((r << 23) | (r >>> 9)) + e) << 0),
        (i = r ^ e),
        (t += (i ^ s) + o[13] + 681279174),
        (t = (((t << 4) | (t >>> 28)) + r) << 0),
        (s += (i ^ t) + o[0] - 358537222),
        (s = (((s << 11) | (s >>> 21)) + t) << 0),
        (h = s ^ t),
        ...
        (t += (i ^ s) + o[9] - 640364487),
        (t = (((t << 4) | (t >>> 28)) + r) << 0),
        (s += (i ^ t) + o[12] - 421815835),
        (s = (((s << 11) | (s >>> 21)) + t) << 0),
        (h = s ^ t),
        (e += (h ^ r) + o[15] + 530742520),
        (e = (((e << 16) | (e >>> 16)) + s) << 0),
        (r += (h ^ e) + o[2] - 995338651),
        (r = (((r << 23) | (r >>> 9)) + e) << 0),
        (t += (e ^ (r | ~s)) + o[0] - 198630844),
        (t = (((t << 6) | (t >>> 26)) + r) << 0),
        (s += (r ^ (t | ~e)) + o[7] + 1126891415),
        (s = (((s << 10) | (s >>> 22)) + t) << 0),
        (e += (t ^ (s | ~r)) + o[14] - 1416354905),
        (e = (((e << 15) | (e >>> 17)) + s) << 0),
        (r += (s ^ (e | ~t)) + o[5] - 57434055),
        (r = (((r << 21) | (r >>> 11)) + e) << 0),
        (t += (e ^ (r | ~s)) + o[12] + 1700485571),
        (t = (((t << 6) | (t >>> 26)) + r) << 0),
        (s += (r ^ (t | ~e)) + o[3] - 1894986606),
        (s = (((s << 10) | (s >>> 22)) + t) << 0),
        (e += (t ^ (s | ~r)) + o[10] - 1051523),
        (e = (((e << 15) | (e >>> 17)) + s) << 0),
        (r += (s ^ (e | ~t)) + o[1] - 2054922799),
        (r = (((r << 21) | (r >>> 11)) + e) << 0),
        (t += (e ^ (r | ~s)) + o[8] + 1873313359),
        (t = (((t << 6) | (t >>> 26)) + r) << 0),
        (s += (r ^ (t | ~e)) + o[15] - 30611744),
        (s = (((s << 10) | (s >>> 22)) + t) << 0),
        (e += (t ^ (s | ~r)) + o[6] - 1560198380),
        (e = (((e << 15) | (e >>> 17)) + s) << 0),
        (r += (s ^ (e | ~t)) + o[13] + 1309151649),
        (r = (((r << 21) | (r >>> 11)) + e) << 0),
        (t += (e ^ (r | ~s)) + o[4] - 145523070),
        (t = (((t << 6) | (t >>> 26)) + r) << 0),
        (s += (r ^ (t | ~e)) + o[11] - 1120210379),
        (s = (((s << 10) | (s >>> 22)) + t) << 0),
        (e += (t ^ (s | ~r)) + o[2] + 718787259),
        (e = (((e << 15) | (e >>> 17)) + s) << 0),
        (r += (s ^ (e | ~t)) + o[9] - 343485551),
        (r = (((r << 21) | (r >>> 11)) + e) << 0),
        this.first
    ...
    (Md5.prototype.base64 = function () {
      for (var t, r, e, s = '', i = this.array(), h = 0; h < 15; )
        (t = i[h++]),
          (r = i[h++]),
          (e = i[h++]),
          (s +=
