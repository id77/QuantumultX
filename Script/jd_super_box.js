...
function getTask() {
  return new Promise((resolve) => {
    $.get(
      taskUrl('apTaskList', { linkId: linkId, encryptPin: '' }),
      async (err, resp, data) => {
        try {
          if (err) {
              ...
              if (data.success) {
                for (let vo of data.data) {
                  if (vo.taskType === 'BROWSE_SHOP') {
                    ...
                    console.log(
                      `助力任务完成进度${vo.taskDoTimes}/${vo.taskLimitTimes}`
                    );
                  }
                }
              } else {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success) {
              if (share) {
                if (data.data.encryptPin)
                  $.inviteCodes.push(data.data.encryptPin);
...
function taskPostUrl(function_id, body = {}) {
  ...
  return {
    ...
    body: `functionId=${function_id}&body=${escape(
      JSON.stringify(body)
    )}&_t=${t}&appid=activities_platform`,
    headers: {
      Accept: '*/*',
