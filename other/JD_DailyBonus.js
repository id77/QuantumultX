  ...
  switch (stop) {
    case 0:
      ...
      await Promise.all([
        ...
        //   'JDUndies',
        //   '京东商城-内衣',
        //   '4PgpL1xqPSW1sVXCJ3xopDbB1f69'
        // ), //京东内衣馆
        // JDUserSignPre(
        //   stop,
        //   'JDCard',
        //   '京东商城-卡包',
        //   '7e5fRnma6RBATV9wNrGXJwihzcD'
        // ), //京东卡包
        ...
        //   'JDShoes',
        //   '京东商城-鞋靴',
        //   '4RXyb1W4Y986LJW8ToqMK14BdTD'
        // ), //京东鞋靴
        // JDUserSignPre(
        //   stop,
        //   'JDChild',
        //   '京东商城-童装',
        //   '3Af6mZNcf5m795T8dtDVfDwWVNhJ'
        // ), //京东童装馆
        // JDUserSignPre(
        //   stop,
        //   'JDBaby',
      ...
      await Promise.all([
        ...
        //   'JDStory',
        //   '京东失眠-补贴',
        //   'UcyW9Znv3xeyixW1gofhW2DAoz4'
        // ), //失眠补贴
        // JDUserSignPre(
        //   stop,
        //   'JDPhone',
        //   '京东手机-小时',
        //   '4Vh5ybVr98nfJgros5GwvXbmTUpg'
        // ), //手机小时达
        // JDUserSignPre(
        //   stop,
        //   'JDEsports',
        //   '京东商城-电竞',
        //   'CHdHQhA5AYDXXQN9FLt3QUAPRsB'
        // ), //京东电竞
        // JDUserSignPre(
        //   stop,
        //   'JDClothing',
...
function notify() {
  return new Promise((resolve) => {
    try {
      ...
      var Cash =
        merge.TotalCash && merge.TotalCash.TCash
          ? `${merge.TotalCash.TCash}红包`
          : '';
      var Steel =
        merge.TotalSteel && merge.TotalSteel.TSteel
          ? `${merge.TotalSteel.TSteel}钢镚`
          : ``;
      var beans =
        merge.TotalBean && merge.TotalBean.Qbear
          ? `${merge.TotalBean.Qbear}京豆${Steel ? `, ` : ``}`
          : '';
      var Money =
        merge.TotalMoney && merge.TotalMoney.TMoney
          ? `${merge.TotalMoney.TMoney}现金${Cash ? `, ` : ``}`
          : '';
      var Subsidy =
        merge.TotalSubsidy && merge.TotalSubsidy.TSubsidy
          ? `${merge.TotalSubsidy.TSubsidy}金贴${Money || Cash ? ', ' : ''}`
          : '';
      var Tbean = bean ? `${bean.toFixed(0)}京豆${steel ? ', ' : ''}` : '';
      var TSteel = steel ? `${steel.toFixed(2)}钢镚` : '';
      var TCash = cash
        ? `${cash.toFixed(2)}红包${subsidy || money ? ', ' : ''}`
        : '';
      var TSubsidy = subsidy
        ? `${subsidy.toFixed(2)}金贴${money ? ', ' : ''}`
        : '';
      ...
      var three =
        TCash || TSubsidy || TMoney
          ? `【其他奖励】:  ${TCash + TSubsidy + TMoney}\n`
          : ``;
      var four = `【账号总计】:  ${beans + Steel}${
        beans || Steel ? `\n` : `获取失败\n`
      }`;
      var five = `【其他总计】:  ${Subsidy + Money + Cash}${
        Subsidy || Money || Cash ? `\n` : `获取失败\n`
      }`;
      var DName =
        merge.TotalBean && merge.TotalBean.nickname
      ...
      const Name =
        DualKey || OtherKey.length > 1
          ? `【签到号${cnNum[$nobyda.num] || $nobyda.num}】:  ${DName}\n`
          : ``;
      ...
      const disa =
        !notify || amount
          ? `【温馨提示】:  检测到${
              $nobyda.disable ? `上次执行意外崩溃, ` : ``
            }已禁用${
              notify ? `${amount}个` : `所有`
            }接口, 如需开启请前往BoxJs或查看脚本内第118行注释.\n`
          : ``;
      $nobyda.notify(
        '',
        '',
        Name + one + two + three + four + five + disa + notify,
        {
          'media-url':
            $nobyda.headUrl ||
            'https://cdn.jsdelivr.net/gh/NobyDa/mini@master/Color/jd.png',
        }
      );
      $nobyda.headUrl = null;
      if ($nobyda.isJSBox) {
        $nobyda.st =
          (typeof $nobyda.st == 'undefined' ? '' : $nobyda.st) +
          Name +
          one +
          two +
          three +
...
(async function ReadCookie() {
  ...
  const move = CookieMove(
    $nobyda.read(EnvInfo) || Key,
    $nobyda.read(EnvInfo2) || DualKey,
    EnvInfo,
    EnvInfo2,
    EnvInfo3
  );
  ...
  if (DeleteCookie) {
    ...
    throw new Error(
      `Cookie清除${write ? `成功` : `失败`}, 请手动关闭脚本内"DeleteCookie"选项`
    );
  } else if ($nobyda.isRequest) {
    GetCookie();
  } else if (Key || DualKey || (OtherKey || cookieSet || '[]') != '[]') {
    if (($nobyda.isJSBox || $nobyda.isNode) && stop !== '0')
      $nobyda.write(stop, 'JD_DailyBonusDelay');
    ...
    if ((OtherKey || cookieSet || '[]') != '[]') {
      try {
        OtherKey = checkFormat([
          ...JSON.parse(OtherKey || '[]'),
          ...JSON.parse(cookieSet || '[]'),
        ]);
        const updateSet = OtherKey.length
          ? $nobyda.write(JSON.stringify(OtherKey, null, 2), EnvInfo3)
          : '';
        ...
        try {
          if (error) {
            ...
            if (cc.code == 3) {
              ...
              console.log('\n' + '京东商城-转盘签到成功 ' + Details);
              merge.JDTurn.bean +=
                (cc.prizeName && cc.prizeName.split(/(\d+)/)[1]) || 0;
              merge.JDTurn.notify += `${also ? `\n` : ``}京东商城-转盘: ${
                also ? `多次` : `成功`
              }, 明细: ${merge.JDTurn.bean || `无`}京豆 🐶`;
        ...
        try {
          ...
          if (
            ...
            if (
              ...
              const ng =
                (cc.resultData && cc.resultData.resBusiMsg) || cc.resultMsg;
              merge.JRSteel.notify = `京东金融-钢镚: 失败, ${`原因: ${
                ng || `未知`
              }`} ⚠️`;
            }
          }
...
function JingDongShake(s) {
  if (!merge.JDShake)
    (merge.JDShake = {}),
      (merge.JDShake.success = 0),
      (merge.JDShake.bean = 0),
  ...
  return new Promise((resolve, reject) => {
      ...
      try {
        if (error) {
          ...
          const turnTableId =
            od.qxTid ||
            (od.floorList || [])
              .filter((o) => o.boardParams && o.boardParams.turnTableId)
              .map((o) => o.boardParams.turnTableId)
              .pop();
    ...
    (data) => {
      ...
      if (typeof data == 'string')
        return JDUserSignPre1(s, key, title, acData, data);
    },
    () => disable(key, title, 2)
  );
}
          ...
          if (error) {
            ...
            if (cc.busiCode == '0') {
              ...
              if (data.match(/(\"busiCode\":\"1002\"|完成签到)/)) {
                ...
                merge.JDCash.notify = `京东现金-红包: 失败, ${
                  msg || `原因: 未知`
                } ⚠️`;
              }
            }
  ...
  return new Promise((resolve, reject) => {
    ...
    const JDUrl = {
      url: `https://api.m.jd.com/client.action?functionId=getNewsInteractionInfo&appid=smfe${
        sign ? `&body=${encodeURIComponent(`{"sign":${sign}}`)}` : ``
      }`,
      headers: {
        Cookie: KEY,
      },
        ...
        url: `https://api.m.jd.com/client.action?functionId=getNewsInteractionLotteryInfo&appid=smfe${
          id
            ? `&body=${encodeURIComponent(
                `{${id.sign ? `"sign":${id.sign},` : ``}"interactionId":${
                  id.id
                }}`
              )}`
            : ``
        ...
        try {
          if (error) {
            ...
            if (
              cc.resultCode == 0 &&
              cc.resultData.data &&
              cc.resultData.data.thisAmount
            ) {
              console.log('\n' + '京东商城-金贴签到成功 ' + Details);
              merge.subsidy.subsidy = cc.resultData.data.thisAmountStr;
              merge.subsidy.notify = `京东商城-金贴: 成功, 明细: ${
                merge.subsidy.subsidy || `无`
              }金贴 💰`;
              ...
              if (data.match(/已存在|"thisAmount":0/)) {
                ...
                const msg = data.split(/\"msg\":\"([\u4e00-\u9fa5].+?)\"/)[1];
                merge.subsidy.notify = `京东商城-金贴: 失败, ${
                  msg || `原因: 未知`
                } ⚠️`;
              }
            }
        ...
        try {
          if (error) {
            ...
            if (cc.resultCode == 0) {
              if (cc.resultData.data.businessData != null) {
                ...
                if (cc.resultData.data.businessData.pickStatus == 2) {
                  if (data.match(/\"rewardPrice\":\"\d.*?\"/)) {
                    const JRDoll_bean = data.match(
                      /\"rewardPrice\":\"(\d.*?)\"/
                    )[1];
                    const JRDoll_type = data.match(
                      /\"rewardName\":\"金贴奖励\"/
                    )
                      ? true
                      : false;
                  ...
                  if (!data.match(/"businessCode":"30\dss?q"/)) {
                    ...
                    const tp = data.match(/已领取|300ss?q/)
                      ? `已签过`
                      : `${ep || es || cc.resultMsg || `未知`}`;
                ...
                merge[key].success = 1;
                merge[key].notify = `${title}: 成功, 明细: ${
                  (award ? num : merge[key].bean) || `无`
                }${award ? `金贴 💰` : `京豆 🐶`}`;
              } else {
                console.log(`\n${title}领取异常 ${Details}`);
                if (num)
                  console.log(
                    `\n${title} 请尝试手动领取, 预计可得${num}${
                      award ? `金贴` : `京豆`
              ...
              console.log(`\n${title}签到失败 ${Details}`);
              const redata =
                typeof cc.resultData == 'string' ? cc.resultData : '';
              merge[key].notify = `${title}: 失败, ${
                cc.resultCode == 3
                  ? `原因: Cookie失效‼️`
                  : `${redata || '原因: 未知 ⚠️'}`
        ...
        (error, response, data) => {
          try {
            ...
            if (cc.data && cc.data.success === true && cc.data.bizCode === 0) {
              ...
              merge.JDGStore.notify = `京东商城-超市: 成功, 明细: ${
                merge.JDGStore.bean || `无`
              }京豆 🐶`;
            } else {
              if (!cc.data) cc.data = {};
              console.log(`\n京东商城-超市签到失败 ${Details}`);
              const tp =
                cc.data.bizCode == 811
                  ...
                  ? `Cookie失效`
                  : `${cc.data.bizMsg || `未知`}`;
              merge.JDGStore.notify = `京东商城-超市: 失败, 原因: ${tp}${
                cc.data.bizCode == 300 ? `‼️` : ` ⚠️`
              }`;
          ...
          (error, response, data) => {
            try {
              ...
              if (cc.code == 0 && cc.subCode == 0) {
                ...
                merge.JDSecKill.notify = `京东秒杀-红包: 成功, 明细: ${
                  merge.JDSecKill.Cash || `无`
                }红包 🧧`;
              } else {
                console.log(`\n京东秒杀-红包签到失败 ${Details}`);
                merge.JDSecKill.fail = 1;
                merge.JDSecKill.notify = `京东秒杀-红包: 失败, ${
                  cc.subCode == 103
                    ? `原因: 已领取`
                    : cc.msg
                    ? cc.msg
    ...
    if (k.length == 2) {
      if ((n = k[1]) && !c[n]) {
        i.userName = i.userName
          ? i.userName
          : decodeURIComponent(n.split(/pt_pin=(.+?);/)[1]);
        i.cookie = k.join('');
...
function CookieUpdate(oldValue, newValue, path = 'cookie') {
  ...
  try {
    ...
    $nobyda.notify(
      '京东签到',
      '',
      'Cookie JSON格式不正确, 即将清空\n可前往日志查看该数据内容!'
    );
    console.log(
      `京东签到Cookie JSON格式异常: ${e.message || e}\n旧数据内容: ${total}`
    );
    total = [];
  }
  for (let i = 0; i < total.length; i++) {
    if (
      total[i].cookie &&
      new RegExp(`pt_pin=${name};`).test(total[i].cookie)
    ) {
      item = i;
      break;
    }
...
function GetCookie() {
  ...
  if (req.method != 'OPTIONS' && req.headers) {
    ...
    if (
      /^https:\/\/(me-|)api(\.m|)\.jd\.com\/(client\.|user_new)/.test(req.url)
    ) {
      if (ckItems && ckItems.length == 2) {
        const value = CookieUpdate(null, ckItems.join(''));
        if (value.type !== -1) {
          const write = $nobyda.write(
            JSON.stringify(value.total, null, 2),
            'CookiesJD'
          );
          $nobyda.notify(
            `用户名: ${value.name}`,
            ``,
            `${value.type == 2 ? `更新` : `写入`}京东 [账号${
              value.item
            }] Cookie${write ? `成功 🎉` : `失败 ‼️`}`
          );
        } else {
          console.log(
      ...
      /^https:\/\/ms\.jr\.jd\.com\/gw\/generic\/hy\/h5\/m\/appSign\?/.test(
        req.url
      ) &&
      req.body
    ) {
