...
const step = randomFriendPin(
  $.getData('xmMinStep') * 1 || 20000,
  $.getData('xmMaxStep') * 1 || 25000
);
function getToken() {
  if ($response.body) {
...
async function start() {
  login_token = $.isNode()
    ? process.env.XM_SPORT_TOKEN
      ? process.env.XM_SPORT_TOKEN
      : login_token
    : $.getData('xmSportsToken')
    ? $.getData('xmSportsToken')
    : login_token;
    ...
    if ($.tokenInfo && $.tokenInfo.result === 'ok') {
      ...
      if ($.changeStepRes && $.changeStepRes.code === 1) {
        ...
        $.msg(
          $.name,
          `${step}步🏃修改成功`,
          `时间：${timeFormat(localtime())}‍`,
          { 'open-url': 'alipays://platformapi/startapp?appId=20000869' }
        );
      } else {
