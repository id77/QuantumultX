...
!(async () => {
  ...
  $.signs.sort(function (a, b) {
    return a.completeNumber - b.completeNumber;
  });

  for (let i = 0; i < $.signs.length; i++) {
    const signInfo = $.signs[i];
    const { isSign, rewardCoin, completeNumber } = signInfo;
    if (isSign === 3) {
      console.log('今天已签到');
      $.desc = `签到失败：今日已签到‼️ 无忧币 +${rewardCoin}`;
      break;
    }
    if (isSign === 2) {
      console.log('签到中');
      ...
      if ($.sign.bizCode == 0) {
        if ($.sign.rewardType != '') {
          // 解析神秘大奖
          $.rewardData = reslutionExtraReward($.sign).replace(
            '无忧币',
            `无忧币 +${rewardCoin}`
          );
        } else {
          // 正常签到奖励
          $.rewardData = `无忧币 +${rewardCoin}`;
        }
        $.desc = `签到成功：已连签${completeNumber}天，${$.rewardData}🎉`;
      } else {
        $.desc = `签到失败，请稍后重试`;
      }
      break;
  ...
  for (var i = 0; i < $.everydayTaskList.length; i++) {
    let taskInfo = $.everydayTaskList[i];

    if (taskInfo.status == 0) {
      //未领取任务
      await receiveTask();

      if ($.receiveTask.code == 0) {
        taskInfo.status = 1;
        //如果限量类型任务
        if (taskInfo.restrictType == 3) {
          taskInfo.finishNumber++;
        }
        //浏览活动、参与活动
        // this.sendEvent(taskInfo);
        // this.jumpUrl(taskInfo);
      } else {
        // alert(res.msg || '领取任务失败');
        // this.promptRefreshPage(); // 提示刷新页面
      }
    } else if (taskInfo.status == 1) {
      //已领取任务
      // this.sendEvent(taskInfo);
      // this.jumpUrl(taskInfo);
    } else if (taskInfo.status == 2 || taskInfo.status == 4) {
      //已完成任务
      // 2.限时任务 3.限量任务
      if (taskInfo.restrictType == 2 || taskInfo.restrictType == 3) {
        // this.jumpUrl(taskInfo);
      }
    }
  }
...
function querySign() {
  return new Promise((resolve, reject) => {
    const url = `https://app.jegotrip.com.cn/api/service/v1/mission/sign/querySign?token=${token}&h_token=${token}&lang=zh_CN`;
...
function sign(signInfo) {
  return new Promise(async (resolve) => {
    const url = `https://app.jegotrip.com.cn/api/service/v1/mission/sign/userSign?token=${token}&h_token=${token}&lang=zh_CN`;
...
function queryEveryDataTask() {
  return new Promise((resolve, reject) => {
    ...
    $.post(options, (err, resp, data) => {
      try {
        let res = JSON.parse(data);
        $.everydayTaskList = res.body.map(
          (item) => (item.isTaskExpalin = false)
        );
      } catch (err) {
        console.log(err);
      } finally {
...
function Stream() {
  ...
  function update() {
    ...
    /** @type {string} */
    var url = jsspicepubmedabstract + harFileURL;
    var previewKey = $.CryptoJS.MD5(url).toString().toLowerCase().slice(8, 24);
    var c = $.CryptoJS.enc.Utf8.parse(
      viewerURL + ';' + harFileURL + ';' + inputLocation
    );
    return {
      key: previewKey,
  ...
  function decode(a) {
    ...
    var expRecords = $.CryptoJS.enc.Base64.parse(a)
      .toString($.CryptoJS.enc.Utf8)
      .split(';');
    if (expRecords && 3 === expRecords.length) {
      var url = jsspicepubmedabstract + expRecords[1];
      samples = $.CryptoJS.MD5(url).toString().toLowerCase().slice(8, 24);
    }
    return samples;
  }
  /**
  ...
  function encrypt(data) {
    ...
    var salt = $.CryptoJS.enc.Utf8.parse(obj.key);
    var whisperMessage = $.CryptoJS.AES.encrypt(key, salt, {
  ...
  function decrypt(val, key) {
    ...
    var ciphertext = $.CryptoJS.enc.Utf8.parse(decode(val));
    ...
    try {
      /** @type {*} */
      text = JSON.parse(item);
    } catch (c) {}
    return text;
  }
  var viewerURL = $.secretKey || 'test1';
  var jsspicepubmedabstract = $.secretVal || '12345678';
  /** @type {string} */
  var inputLocation = '01';
  return {
    Encrypt: encrypt,
    Decrypt: decrypt,
  };
